...
contract AaveAggregator {
    ...
    function deposit(uint256 amount) external {
        ...
        uint256 share = (totalShare == 0) ? amount : (totalShare * amount) / prevBalance;
    ...
    /**
     * @dev Withdraw DAI from the Aave pool based on the user's shares.
     * @param share The amount of shares to withdraw.
     * @notice The caller must have enough shares to withdraw the requested amount.
     */
    function withdraw(uint256 share) external {
        require(share > 0, "Nothing to withdraw");
        require(share <= userShare[msg.sender], "Withdraw amount exceeds balance");
    ...
    function _balanceOf() internal view returns (uint256) {
        address aDaiAddress = aavePool.getReserveData(address(dai)).aTokenAddress; // Get aToken address
